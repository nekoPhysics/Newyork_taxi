...
def lern_cycle(train_df):
    ...
    x = df_imp['month_cos']
    y = df_imp['month_sin']
    plt.scatter(x, y, c='c', label="plot", alpha=1)
    plt.title("Convert month data using trigonometric functions")
    plt.savefig('month_tri.png')
    plt.show()
...
df = df.sample(n=100, random_state=777)
